...
## Colors
`color` lets you select a hue from the predefined semantic color palette and `prominence` is the available shades within that hue. Use `customColor` to apply colors outside of the predefined color sets (any valid css color or css custom property containing a valid css color). Define `--override-text-color` in a parent to override all text color within.
